...
if (mysqli_query($conn, $sql_create)) {
      ...
      $i= 7;
      if ($excel->getActiveSheet()->getCell('A7')->getValue() == "1") {
      ...
      if ($insert) 
      {
          array_push($msgs, 'You have successfully uploaded a record for '.$course_code.' result.');
      } 
      else {
          array_push($errors, 'Uploading a record for '.$course_code.' result NOT successfull');
      } 
      }
      

      else{
        array_push ($errors, 'ERROR!!!... Please check the file to ensure that serial number starts from cell number "7"');
      }
          }
mysqli_close($conn);
    ...
    ?>
<?php $site = "lecturer_upload_result.php?logout='1'"; include 'lecturer_header.php';?>
<form enctype="multipart/form-data" class="form-group" action="lecturer_upload_result.php" method="post">
  <div class="alert alert-danger"><?php echo display_error();?></div>
  <P><?php display_msg(); ?></P>
    <?php $header = "Upload Course-Based Result"; include 'body.php';?>
